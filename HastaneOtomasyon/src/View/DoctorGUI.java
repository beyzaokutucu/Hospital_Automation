...
public class DoctorGUI extends JFrame {
	...
	public DoctorGUI(Doctor doctor) {
		...
		whourData = new Object[3];//0, 1, 2
		
		try {
			for (int i = 0; i < doctor.getWhourList(doctor.getId()).size(); i++) {
				whourData[0] = doctor.getWhourList(doctor.getId()).get(i).getId();
				whourData[1] = doctor.getWhourList(doctor.getId()).get(i).getDoctor_name();
				whourData[2] = doctor.getWhourList(doctor.getId()).get(i).getWdate();
		...
		w_appointments.setBackground(SystemColor.control);
