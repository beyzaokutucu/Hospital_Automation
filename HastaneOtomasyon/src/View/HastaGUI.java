...
public class HastaGUI extends JFrame {
	...
	public HastaGUI(Hasta hasta) throws SQLException {
		...
		w_appointment.setBackground(SystemColor.control);
		...
		lbldoctorsec.setForeground(SystemColor.textHighlight);
		...
		lblUygunSaatler.setForeground(Color.DARK_GRAY);
		...
		lblRandevu.setForeground(SystemColor.textHighlight);
		...
		btn_addAppoint.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int selRow = table_whour.getSelectedRow();//seçim yapýp index buluyor
				Helper.showMsg(selRow + " cell");
				String date = "";
				boolean allCorrect =true; 
				try
				{
					date = table_whour.getModel().getValueAt(selRow, 1).toString();
					Helper.showMsg("Date: " + date);
				}
				catch(Exception e1)
				{
					Helper.showMsg("You must to select date");
					allCorrect = false;
				}
				try
				{
					hasta.getName();
					if(hasta.getName() == null)
					{
						throw new Exception();
					}
					Helper.showMsg("Hasta name: " + hasta.getName());
				}
				catch(Exception e1)
				{
					Helper.showMsg("Hasta name can't be null");
					allCorrect = false;
				}
				try
				{
					...
					Helper.showMsg("Hasta id: " + hasta.getId());
				}
				catch(Exception e1)
				{
					Helper.showMsg("Hasta ID can't be null");
					allCorrect = false;
				}
				Helper.showMsg("Doctor name: " + selectDoctorName);
				
				if(selectDoctorName == null)
				{
					Helper.showMsg("doctor name can't be null");
					allCorrect = false;
				}
				if(allCorrect == true)
				{
