...
public class LoginGUI extends JFrame {
	...
	public LoginGUI() {
		...
		JLabel lbl_logo = new JLabel(new ImageIcon(LoginGUI.class.getResource("hospital.png")));
		lbl_logo.setBounds(188, 11, 122, 123);
		w_pane.add(lbl_logo);

		JLabel lblHastaneYnetimSistemine = new JLabel("Hastane Yonetim Sistemine Hosgeldiniz");
		lblHastaneYnetimSistemine.setForeground(SystemColor.window);
		...
		w_hastaLogin.setBackground(SystemColor.control);
		...
		w_doctorLogin.setBackground(SystemColor.control);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (fld_doktorTc.getText().length() == 0 || fld_doktorPass.getText().length() == 0) {
					...
					try {
						...
						ResultSet rs = st.executeQuery("SELECT *FROM user");
						while (rs.next()) {
							if (fld_doktorTc.getText().equals(rs.getString("tcno")) && fld_doktorPass.getText().equals(rs.getString("pasword"))) {
								...
								if (rs.getString("type").equals("bashekim")) {
									...
									doctor.setPasword(rs.getString("pasword"));
